...
from toko.stand05 import stand05
from toko.elfood import elfood
import locale
...
class LoginWindow:
    ...
    def create_widgets(self):
        ...
        try:
            image = Image.open("images/unesalogo.png")
            image = image.resize((100, 100))
            photo = ImageTk.PhotoImage(image)
            logo = tk.Label(top_frame, image=photo)
            logo.image = photo
    ...
    def login(self):
        ...
        if username in admin_accounts:
            if username in users and users[username]["password"] == password:
                ...
                messagebox.showerror("Error", "Salah memasukkan password/email admin!")
            return
...
class pilih_kedai:
    ...
    def open_stand05(self):
        """ Fungsi untuk membuka window Teh Poci """
        self.root.withdraw()
        root = tk.Toplevel()
        app = stand05(root, self.username, self.balance, self.root)
        root.mainloop()  

    def open_elfood(self):
        """ Fungsi untuk membuka window Teh Poci """
        self.root.withdraw()
        root = tk.Toplevel()
        app = elfood(root, self.username, self.balance, self.root)
        root.mainloop()      
